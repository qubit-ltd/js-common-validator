...
//
////////////////////////////////////////////////////////////////////////////////
import { PasswordRule } from '@qubit-ltd/validation-rule';
 ...
 * @author Haixing Hu
 */
function validatePasswordField(value, context = {}) {
  context.label ??= '密码';
  context.minLength ??= PasswordRule.minLength;
  context.maxLength ??= PasswordRule.maxLength;
  context.invalidMessage ??= '{whose}{label}只能由大小写英文字母、阿拉伯数字、非空白ASCII符号组成';
  context.tooShortMessage ??= '{whose}{label}不能少于{min}个字符';
  context.tooLongMessage ??= '{whose}{label}不能多于{max}个字符';
  return validateFieldByRule(value, PasswordRule, context);
}
